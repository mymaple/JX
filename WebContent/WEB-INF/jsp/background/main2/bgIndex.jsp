...
<base href="<%=basePath%>">
	...
	<!-- 即时通讯 -->
	<script type="text/javascript">var wimadress="${pd.instantChatIp}:${pd.instantChatPort}";</script>
	<script type="text/javascript">var oladress="${pd.onlineManageIp}:${pd.onlineManagePort}";</script>
						...
						<div class="pull-left">
							<select id="skin-colorpicker" class="hidden">
								<option data-class="default" value="#438EB9"
									<c:if test="${bgUser.skin =='default' }">selected</c:if>>#438EB9</option>
								<option data-class="skin-1" value="#222A2D"
									<c:if test="${bgUser.skin =='skin-1' }">selected</c:if>>#222A2D</option>
								<option data-class="skin-2" value="#C6487E"
									<c:if test="${bgUser.skin =='skin-2' }">selected</c:if>>#C6487E</option>
								<option data-class="skin-3" value="#D0D0D0"
									<c:if test="${bgUser.skin =='skin-3' }">selected</c:if>>#D0D0D0</option>
							</select>
						</div>
						<span>&nbsp; 选择皮肤</span>
					</div>
