...
<script type="text/javascript">
	...
	function addMenu(){
		 ...
		 diag.Drag=true;
		 diag.Title ="新增菜单";
		 diag.URL = '<%=basePath%>background/menu/toAddMenu.do';
		 diag.Width = 223;
		 diag.Height = 256;
	...
	function editMenu(menuId){
		 ...
		 diag.Drag=true;
		 diag.Title ="编辑菜单";
		 diag.URL = '<%=basePath%>background/menu/toEditMenu.do?menuId='+menuId;
		 diag.Width = 223;
		 diag.Height = 256;
		 diag.CancelEvent = function(){ //关闭事件
			if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
	...
	function deleteMenu(menuId,isParent){
		...
		if(flag){
			top.jzts();
			var url = "<%=basePath%>background/menu/deleteMenu.do?menuId="+menuId+"&guid="+new Date().getTime();
	...
	function openClose(menuId,curObj,trIndex){
		...
		if(txt=="展开"){
			...
			$.ajax({
				type: "POST",
				url: '<%=basePath%>background/menu/submenuList.do',
		    	data: {menuId:menuId,tm:new Date().getTime()},
				dataType:'json',
				cache: false,
				success: function(data){
					if(data.subMenuList.length>0){
						var html = "";
						$.each(data.subMenuList,function(i){
							...
							html += "<td><a class='btn btn-mini btn-info' title='编辑' onclick='editMenu(\""+this.menuId+"\")'><i class='icon-edit'></i></a> <a class='btn btn-mini btn-danger' title='删除' onclick='deleteMenu(\""+this.menuId+"\",false)'><i class='icon-trash'></i></a></td>";
							html += "</tr>";
	...
	<table id="table_report" class="table table-striped table-bordered table-hover">
		...
		<c:choose>
			<c:when test="${not empty menuList}">
				...
				<a class='btn btn-mini btn-purple' title="图标" onclick="toChangeMenuIcon('${menu.menuId }')" ><i class='icon-picture'></i></a>
				<a class='btn btn-mini btn-info' title="编辑" onclick="editMenu('${menu.menuId }')" ><i class='icon-edit'></i></a>
				<a class='btn btn-mini btn-danger' title="删除"  onclick="deleteMenu('${menu.menuId }',true)"><i class='icon-trash'></i></a>
				</tr>
				</c:forEach>
			</c:when>
			<c:otherwise>
